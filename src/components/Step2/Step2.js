...
import image from '../../img/Step2/Step2.jpg'
                            ...
                            <Card className={classes.card}>
                                <CardContent className={classes.cardContent}>
                                    <Box display="flex" flexDirection="column" className={classes.section}>
                                        <h3 className={classes.subtitle}>Материал для внутренней отделки: </h3>

                                        <FormControl component="fieldset" className={classes.formControl}>
                                                <RadioGroup aria-label="position" name="position" defaultValue="1">
                                                    { Object.keys(house.step2).map( el => <FormControlLabel
                                                        key = {house.step2[el].name}
                                                        checked={house.step2[el].value === 1}
                                                        value={house.step2[el].name}
                                                        control={<Radio />}
                                                        label={house.step2[el].rusName}
                                                        onChange={e => CState.changeInterior(house.step2, house.step2[el])}
                                                        labelPlacement="end"
                                                    /> ) }
                    ...
                    <MobileStepper
                        ...
                        nextButton={
                            <StepButton 
                                ...
                                disabled={ !Object.values(house.step2).filter( el => el.value ).length }
                            />
                        }
                        backButton={
                            <StepButton 
...
const mapDispatchToProps = dispatch => ({
    ...
    CState: {
        changeInterior: (interior, type) => dispatch(ACTIONS.setInterior(interior, type)),
